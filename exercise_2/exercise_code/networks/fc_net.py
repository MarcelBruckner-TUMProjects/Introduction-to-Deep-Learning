...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, num_layers):
            ...
            if self.use_batchnorm:
                out, cache[i] = affine_batchnorm_relu_forward(
                    out, self.params['W' + x], self.params['b' + x],
                    self.params['gamma' + x], self.params['beta' + x],
                    self.bn_params[i - 1])
            else:
                out, cache[i] = affine_relu_forward(out, self.params['W' + x], self.params['b' + x])

            if self.use_dropout:
                out, dropout_cache = dropout_forward(out, self.dropout_param)
                cache[i] = (cache[i], dropout_cache)

        scores, cache[num_layers] = affine_forward(out, self.params['W' + str(num_layers)], self.params['b' + str(num_layers)])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        loss += 0.5 * reg * np.sum(self.params['W'+str(num_layers)] * self.params['W'+str(num_layers)])
        ...
        for i in range(num_layers - 1, 0, -1):
            ...
            if self.use_batchnorm:
                dx, dw, grads['b' + x], grads['gamma' + x], grads['beta' + x] = affine_batchnorm_relu_backward(dx, cache[i])
            else:
                dx, dw, grads['b' + x] = affine_relu_backward(dx, cache[i])
            
            grads['W' + x] = dw + reg * self.params['W' + x]
            loss += 0.5 * reg * np.sum(self.params['W'+x] * self.params['W'+x])
