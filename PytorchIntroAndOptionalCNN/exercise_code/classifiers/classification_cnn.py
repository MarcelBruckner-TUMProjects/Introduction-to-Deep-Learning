...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        self.conv1 = nn.Conv2d(channels, num_filters, kernel_size,
                               stride=stride_conv, padding=int(kernel_size / 2), padding_mode='same', bias=True)
        self.conv1.weight.data.mul_(weight_scale)
        self.fc1 = nn.Linear(int(height / pool) * int(width / pool) * num_filters, hidden_dim, bias=True)
